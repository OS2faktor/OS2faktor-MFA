...
namespace Login
{
    public class CredentialProviderCredential : ICredentialProviderCredential
    {
        ...
        public CredentialProviderCredential(CredentialView view)
        {
            if (!initialized)
            {
                try
                {
                    ...
                    appender.File = "c:/logs/os2faktor/login.log";
                ...
                catch (Exception ex)
                {
                    try
                    {
                        System.IO.File.WriteAllText(@"c:\logs\os2faktor\login-fail.log", "failure: " + ex.Message + "\n" + ex.StackTrace);
                    }
                    catch (Exception)
                    {
                        ; // ignore
        ...
        )
        {
            ...
            var result = OS2Faktor.InitializeOS2faktorFlow(username,password);
            //cancel  - close window or cancel in nice way
            //abort  - error
            //OK,YES - os2faktor login succeded
            switch (result)
            {
                ...
                case DialogResult.Abort:
                    //Username pass was bad
                    pcpgsr = _CREDENTIAL_PROVIDER_GET_SERIALIZATION_RESPONSE.CPGSR_NO_CREDENTIAL_NOT_FINISHED;
                    pcpcs = new _CREDENTIAL_PROVIDER_CREDENTIAL_SERIALIZATION();
                    ppszOptionalStatusText = "Forkert brugernavn eller kodeord.";
                ...
                case DialogResult.Cancel:
                    //TODO maybe we should clear the username password box?
                    break;
                default:
                    break;
            }
