	...
	public PidAndCprOrError verify(String responseB64, HttpServletRequest request) {
		...
		if ("ok".equals(result)) {
			try {
				...
				if (challenge == null) {
					throw new ChallengeDoesNotMatchException("no challenge on session");
				}
				...
				if (certificateAndStatus.getCertificateStatus() != CertificateStatus.VALID) {
					return new PidAndCprOrError(ErrorType.BAD_CERTIFICATE, "Certificate status is: " + certificateAndStatus.getCertificateStatus());
				}
				...
				return new PidAndCprOrError(ErrorType.NOT_POCES, "Certificate is not FOCES");
			}
			catch (ChallengeDoesNotMatchException ex) {
				log.warn("Caught ChallengeDoesNotMatchException: " + ex.getMessage());
				
				return new PidAndCprOrError(ErrorType.EXCEPTION, ex.getMessage());
			}
			catch (Exception ex) {
		...
		else if (ControllerUtil.knownNemIDError(result)) {
			return new PidAndCprOrError(ErrorType.NEMID_VALIDATION_KNOWN_ERROR, result);
		}
